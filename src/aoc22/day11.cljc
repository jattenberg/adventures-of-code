        ...
        pass-item-fn (fn [ms is]
                       (if (= (count is) 0)
                         ...
                         (let [i (first is)
                               ...
                               with-boredem new-score ; (quot new-score 3) ; divide by 3
...
(defn print-and-return
  ...
  (doseq [i monkeys] (println (str "Monkey " (:m-id i) ": " (:items i))))
  monkeys)
...
(defn part-1
  ...
  (let [input (->> (io/resource "aoc22/day11.txt")
        ...
        ct 20
        round1 (play-all-monkeys monkeys)
...
(defn part-2
  ...
  [_]
    (let [input (->> (io/resource "aoc22/day11example.txt")
                   (slurp)
                   (str/split-lines)
                   (map str/trim)
                   (partition-by str/blank?)
